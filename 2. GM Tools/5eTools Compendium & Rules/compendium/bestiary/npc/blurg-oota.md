---
obsidianUIMode: preview
cssclasses: json5e-monster
tags:
- compendium/src/5e/oota
- monster/cr/2
- monster/size/medium
- monster/type/humanoid/orc
aliases: ["Blurg"]
---
# Blurg
*Source: Out of the Abyss p. 29, Derived from *  

```ad-statblock
title: Blurg
![](/compendium/bestiary/npc/token/blurg.png#token)
*Medium humanoid (orc), Neutral*

- **Armor Class** 18  ([plate armor](/compendium/items/plate-armor.md))
- **Hit Points** 42 (`5d8 + 20`)
- **Speed** 30 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|18 (+4)|12 (+1)|18 (+4)|18 (+4)|11 (+0)|12 (+1)|

- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** Intimidation +5, Survival +2
- **Senses** darkvision 60 ft., passive Perception 10
- **Languages** Common, Orc, Dwarvish, Elvish, Undercommon
- **Challenge** 2

## Traits

***Aggressive.*** As a bonus action, Blurg can move up to its speed toward a hostile creature that it can see.

***Innate Spellcasting.*** Blurg can innately cast can cast the [teleport](/compendium/spells/teleport.md) spell once per day, but the intended destination must be within 30 feet of another society member. This teleport effect can be disrupted (see "Faerzress"), which is how society members sometimes end up in far corners of the Underdark, separated from their fellows.

**1/day each**: [teleport](/compendium/spells/teleport.md)

## Actions

***Multiattack.*** Blurg makes two greataxe attacks.

***Greataxe.*** *Melee Weapon Attack:* +6 to hit, reach 5 ft., one target. *Hit:* 10 (`1d12 + 4`) slashing damage.

***Javelin.*** *Melee or Ranged Weapon Attack:* +6 to hit, reach 5 ft. or range 30/120 ft., one target. *Hit:* 7 (`1d6 + 4`) piercing damage.
```
^statblock